...
export function Create() {
  ...
  async function newPoll(e) {
    if(optThree== ''){
      ...
      api.post('/polls', {
        title,
        options: [optOne, optTwo, optThree, optFour, optFive]
      }).then((response) => {
        console.log(response)
      })
    }
  ...
  return (
    <div>
      ...
      <form className="ml-9 mt-16">
        ...
        <button onClick={newPoll} disabled={!title || !optOne || !optTwo || (selected == 3) && !optThree || (selected == 4) && !optFour || (selected == 5) && !optFive} className="mt-10 w-72 bg-rose-50 border-2 border-rose-900 rounded-xl h-12 font-bold text-xl enabled:hover:bg-rose-900 enabled:hover:text-rose-50 enabled:focus:bg-rose-800 enabled:focus:text-rose-50 disabled:opacity-25"><h1>CRIAR</h1></button>
      </form>
    </div>
  )
}